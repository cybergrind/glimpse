...
mod tests {
    ...
    fn manual_schedule_supports_overnight_windows() {
        ...
        let evaluation = evaluate_manual_schedule(&window, "23:30").expect("evaluation");

        assert_eq!(evaluation.phase, NightLightPhase::Night);
        assert_eq!(evaluation.night_progress, 1.0);
    }
    ...
    fn automatic_schedule_uses_day_between_sunrise_and_sunset() {
        ...
        let evaluation = evaluate_automatic_schedule(&window, "12:00").expect("evaluation");

        assert_eq!(evaluation.phase, NightLightPhase::Day);
        assert_eq!(evaluation.night_progress, 0.0);
    }
