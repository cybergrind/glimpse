...
fn managed_include_runtime_path(kind: CompositorKind) -> Option<PathBuf> {
    let home = std::env::var_os("HOME")?;
    let mut path = PathBuf::from(home);
    match kind {
        CompositorKind::Niri => {
            path.push(".config/niri/glimpse.d/index.kdl");
            Some(path)
        }
        CompositorKind::Hyprland | CompositorKind::Unknown => None,
    }
}
...
fn apply_niri_persisted_displays(draft: &DisplayDraft) -> Result<PersistStatus, String> {
    ...
    let Some(index_path) = managed_include_runtime_path(CompositorKind::Niri) else {
        return Err("HOME is not set".into());
    };
    let parent = path
        .parent()
    ...
    fs::write(&index_path, serialize_niri_index()).map_err(|error| error.to_string())?;

    let include_present = detect_niri_include(&index_path);
    finalize_niri_apply(
        path,
        include_present,
...
fn detect_niri_include(managed_path: &Path) -> bool {
...
fn validate_niri_config(path: &Path) -> Result<(), String> {
...
fn reload_niri_config() -> Result<(), String> {
    let output = Command::new("niri")
...
fn serialize_niri_index() -> &'static str {
    "// Generated by Glimpse Settings. Manual edits may be overwritten.\ninclude \"~/.config/niri/glimpse.d/displays.kdl\"\n"
}
...
mod tests {
    ...
    #[test]
    fn detects_niri_include_for_managed_file() {
        let config = r#"
            input {}
            include "~/.config/niri/glimpse.d/index.kdl"
        "#;

        assert!(super::niri_include_present(
            config,
            "~/.config/niri/glimpse.d/index.kdl"
        ));
    }
    ...
    #[test]
    fn serializes_niri_index_to_include_displays_fragment() {
        let text = super::serialize_niri_index();
